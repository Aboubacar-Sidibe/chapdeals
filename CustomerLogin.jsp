                ...
                var lock = new PatternLock('#pattern1',
                        {
                            onDraw: function ()
                            {
                                ...
                                checkUserData(phone, lock.getPattern());
                                lock.reset();
                            }
    ...
    <body onload="updateMail()">
        ...
        <div id="modal"> 
            ...
            <form id="regForm"  class="w3-col s12 m12 l6 w3-display-middle" action="./CustomerLogin" method="post">
                ...
                <h1><fmt:message key="Title"/>:</h1> <span class="w3-right"><a href="CustomerRegistration.jsp" class="w3-text-white"><fmt:message key="SignUpMessage"/></a></span>
                <!-- One "tab" for each step in the form: -->
                <div>
                    <label ><fmt:message key="message"/> </label>
                    <p><input id="phone" placeholder="<fmt:message key="message"/>"  type="number" name="phone" required=""></p>
                ...
                <div>
                    ...
                    <div id="Paris" class="w3-container city">
                        <div class="patter-wrapper w3-card-4">
                            <h3 class="w3-center c-pink"><fmt:message key="Draw"/></h3>
                            <div  id="pattern1">
                            </div>
                    ...
                    <a class="w3-text-white" href="EnterMailForPasswordReset.jsp">Forgot password?</a>
                </div>
                ...
                <input type="hidden" id="mailchecked" name="mailchecked"/>
                <input type="hidden" id="contactchecked" name="contactchecked"/>
        ...
        function checkUserData(phone, password)
        {
            ...
            firebase.database().ref('Users/').orderByChild("phone").equalTo(phone).on("value", snap =>
            {
                if (snap.val() != null) // je pouvais le remplacer par exits()
                {
                    snap.forEach(b => {
                        if ((b.val().phone == phone) && (b.val().password == password))
                        {
                            document.getElementById("username").value = b.val().username;
                            document.getElementById("email").value = b.val().email;
                            document.getElementById("dob").value = b.val().dob;
                            document.getElementById("address").value = b.val().address;
                            document.getElementById("mailchecked").value = b.val().mailchecked;
                            document.getElementById("contactchecked").value = b.val().contactchecked;
            ...
            if (key != "invalid")
            {
                database.ref("Users/" + key).update
                        ...
                        , function (error) {
                            ...
                            {
                                alert("mail verified ✅, you can login now");
                            }
