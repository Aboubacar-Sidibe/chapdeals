    ...
    <body>
        ...
        <script>
            ...
            $(document).ready(function () {
                ...
                const onWishListItem = firebase.database().ref("WishList/" + u);
                onWishListItem.on('value', function (wishListSnapshot)
                {

                    document.getElementById("products").innerHTML = "";

                    wishListSnapshot.forEach(function (wishListChildSnapshot) {
                        var wishListChildData = wishListChildSnapshot.val();
                        const pathToItem = wishListChildData["pathToItem"];
                        var imagesDownloadLinksArray = [];
                        //console.log("pathToItem is :" + pathToItem);

                        const itemRef = firebase.database().ref("Items/" + pathToItem);
                        itemRef.once("value", function (itemRefSnapShot) {
                            var item = itemRefSnapShot.val();

                            var imagesRef = firebase.database().ref("Items/" + pathToItem + "/images");
                            imagesRef.once("value", function (imagesnapshot) {
                                ...
                                div.setAttribute("id", pathToItem);
                                div.setAttribute("class", "w3-col s6 m4 chl1 w3-margin-bottom responsiveImg");
                                div.setAttribute("title", item["smalldescription"]);
                                ...
                                span1.innerHTML = "(" + item["discount"] + "% off)";
                                p.appendChild(span1);

                                let heart = document.createElement("i");
                                heart.setAttribute("class", "fa fa-trash w3-right w3-text-grey w3-large w3-hover-text-theme");
                                heart.setAttribute("title", "remove from whish list");
                                heart.setAttribute("onclick", "removeFromWishList(this.parentNode.parentNode.parentNode.id)");

                                let cart = document.createElement("i");
                                cart.setAttribute("class", "fa fa-shopping-bag w3-right w3-text-grey w3-margin-right w3-large w3-hover-text-theme");
                                cart.setAttribute("title", "move to cart");
                                ...
                                var br = document.createElement("BR");
                                span2.innerHTML = "<strong>" + item["brand"] + "</strong>";
                                p.appendChild(span2);
                                p.appendChild(br);

                                var span3 = document.createElement("span");
                                var br = document.createElement("BR");
                                span3.innerHTML = item["smalldescription"];
                                ...
                                span5.innerHTML = "(" + item["mrp"] + " f)";
                                span4.appendChild(span5);
                                var strong = document.createElement("strong");
                                strong.innerHTML = " " + item["price"] + " f";
            ...
            <c:choose>
                ...
                itemsRef.update({
                    ...
                    {
                        var element = document.getElementById(productFullPath);
            ...
            const removeFromWishList = (fullPathToItem) => {
                let u = document.getElementById("u").value;
                let itemKey = fullPathToItem.split("/")[2];

                firebase.database().ref("WishList/" + u + "/" + itemKey).remove().then(function () {
        ...
        <script src="js/CartWishListCount.js"/>
    </body>
</html>
